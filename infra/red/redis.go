...
func getNewClient(ctx context.Context) (*redis.Client, error) {
	connectionStr := config.Spec.Redis
	options, err := redis.ParseURL(connectionStr)
	if err != nil {
		util.Logger(context.TODO()).Fatal("Failed to Parse Redis Connection String", zap.Error(err))
		return nil, err
	}
	client := redis.NewClient(options)
	_, err = client.Ping(ctx).Result()
	if err != nil {
		util.Logger(context.TODO()).Error("Failed to connect to Redis server", zap.Error(err))
		return nil, err
	}
	...
	if err := redisotel.InstrumentTracing(client); err != nil {
		util.Logger(context.TODO()).Error("Failed to install tracing hook", zap.Error(err))
		return nil, err
	}

	if err := redisotel.InstrumentMetrics(client); err != nil {
		util.Logger(context.TODO()).Error("Failed to install metrics hook", zap.Error(err))
		return nil, err
	}
